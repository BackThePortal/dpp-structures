...
namespace dpp_structures::http {
	...
	enum query_key_type { REQUIRED, OPTIONAL };

	enum query_value_convert {
		OPERATOR_STRING,
		THROUGH_JSON,
	};

	/*
     * The derived classes optional and required are used for specifying which query parameters a method accepts.
     * To pass query values, use query.
     */
	template<internal::string_literal TKey, typename ValueType = std::string,
			 query_value_convert ConvertPolicy = OPERATOR_STRING>
	struct query_base {
		struct [[maybe_unused]] _query_base_tag {};

		static constexpr internal::string_literal Key = TKey;

		explicit query_base(bool b)
			requires std::is_same_v<ValueType, bool>
			: value(b ? "true" : "false") {}

		explicit query_base(ValueType value)
			requires(std::is_same_v<ValueType, std::string>)
			: value(std::move(value)) {};
		explicit query_base(ValueType value)
			requires(!std::is_same_v<ValueType, std::string> && !std::is_same_v<ValueType, bool> &&
					 ConvertPolicy == OPERATOR_STRING)
			: value(std::string(value)) {}
		explicit query_base(ValueType value)
			requires(!std::is_same_v<ValueType, std::string> && !std::is_same_v<ValueType, bool> && ConvertPolicy == THROUGH_JSON)
	...
	struct required : query_base<Key, ValueType, ConvertPolicy> {
		...
		static constexpr query_key_type type() { return REQUIRED; }
	};
	...
	struct optional : query_base<Key, ValueType, ConvertPolicy> {
		...
		static constexpr query_key_type type() { return OPTIONAL; }
	};
	...
	template<typename T>
	concept QueryValue = requires { typename T::_query_tag; };
	template<typename T>
	...
	struct query_value {
		struct [[maybe_unused]] _query_tag {};
		static constexpr internal::string_literal Key = TKey;
		...
		/**
		 * Convert a query into a query_base for use in method. This indirection allows
	...
	template<dpp::http_method HttpMethod, internal::string_literal Name, internal::string_literal LiteralURL,
	...
	private:
		...
		static consteval check_query_parameters_result check_query_parameters() {
			...
			for (const auto& [key, key_type] : allQueryKeys) {
				if (key_type == REQUIRED) { ++requiredKeysNum; }
			}
		...
		template<QueryValue TargetQuery>
		std::string get_query_value(TargetQuery query_key) {
			using candidate_query = find_query_by_key<TargetQuery::Key>;

			return query_key.template get_query<candidate_query>().value;
		}

		template<QueryValue FirstQueryKey, QueryValue... Rest>
		std::map<std::string, std::string> get_query_values(FirstQueryKey first_query_key, Rest... rest_queries) {
			std::map<std::string, std::string> map;
			if constexpr (sizeof...(rest_queries) != 0) map = get_query_values(rest_queries...);
	...
	public:
		...
		template<QueryValue... MyQueryKeys>
		std::string operator()(MyQueryKeys... queryKeys) {
			return this->operator()(path_params{}, queryKeys...);
		}


		template<std::size_t PathParamsLength, QueryValue... MyQueryKeys>
		std::string operator()(const path_params<PathParamsLength>& path_params, MyQueryKeys... query_keys_raw) {
			...
			if constexpr (sizeof...(query_keys_raw) != 0)

			{
				std::map<std::string, std::string> query_params = get_query_values<MyQueryKeys...>(query_keys_raw...);
				...
				for (const auto& [key, value] : query_params) {
					...
					if (!value.empty()) new_url.append("=" + value);
	...
	private:
		...
		dpp::task<dpp::http_request_completion_t> request(const std::string& url, const std::string& body = "",
														  const std::string& contentType = "application/json") {
			co_await this->pre_request(url);
			co_return co_await this->bot->co_request(url, HttpMethod, body, contentType, this->get_default_headers());
		};

	protected:
		using feature::bot;

		virtual dpp::task<void> pre_request(const std::string& url) { co_return; }

		template<internal::string_literal TKey>
		static constexpr auto query(auto value) {
...
*/
         ...
         * @return
         */
		virtual std::multimap<std::string, std::string> get_default_headers() { return {}; }
		...
		dpp::task<dpp::http_request_completion_t> post(resolved_method<Method> resolved_method, nlohmann::json body,
			...
			co_return co_await this->request<dpp::m_post>(join_paths(this->base_url, resolved_method.url), body,
														  contentType);
		};

		template<MethodType Method>
			requires(Method::httpMethod == dpp::http_method::m_get)
		dpp::task<dpp::http_request_completion_t> get(const resolved_method<Method>& resolved_method,
													  nlohmann::json body = "",
													  const std::string& contentType = "application/json") {

			co_return co_await this->request<dpp::m_get>(join_paths(this->base_url, resolved_method.url), body,
														 contentType);
		};
	};
